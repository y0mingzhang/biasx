...
config_template = """output_dir: sweeps/outputs/supervised/{data_split}/t5-3b/{lr}-{acc_steps}
model_name: t5-3b
{data_split_config}
  batch_size: 2
...
os.makedirs("sweeps/configs/supervised/t5-3b", exist_ok=True)
for data_split, data_split_config in data_split_configs.items():
    for lr in LR:
        for acc_steps in ACC_STEPS:
            ...
            with open(f"sweeps/configs/supervised/t5-3b/{data_split}-{lr}-{acc_steps}.yaml", "w") as f:
                f.write(config)
